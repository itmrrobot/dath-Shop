  ...
  return (
    <div className={cx("wrap-manage","g-38")}>
      ...
      <div className={cx("table")}>
        ...
        <div className={cx("table-body")}>
          {data?.products&&data?.products.map((p,index) => {
                return <div className={cx("table-row")} key={index}>
                <span className={cx("table-cell","f-2")}>{p?.ten_san_pham}</span>
                <span className={cx("table-cell","f-1")}>{p?.Category?.ten_chuyen_muc}</span>
                <span className={cx("table-cell","f-1")}>{p?.so_luong_nhap}</span>
                <span className={cx("table-cell","f-1")}>{p?.gia_khuyen_mai?.toLocaleString('it-IT', {style : 'currency', currency : 'VND'})}</span>
                <span className={cx("table-cell","f-1")}>{p?.so_luong_ban}</span>
                <span className={cx("table-cell","f-1")}><img src={arrowDownIcon} alt="icon" onClick={() => toggle(p?.id)}/></span>
                <div className={cx("dropdown-list")} style={open === p?.id? { display: "block"} : { display: "none" }}>
                  <div className={cx("dropdown-item")}>Sửa</div>
                  <div className={cx("dropdown-item")} onClick={() => handleDelete(p?.id)}>Xóa</div>
                </div>
              </div>
          })}
        </div>
      ...
      <div className={cx("right")}>
        <button className={cx("btn-add")} onClick={() => navigate('/admin/products/add')}>+ Add new product</button>
      </div>
    </div>
  );
}
